...
describe('el sondeo de permisos no re-navega si nada cambió', () => {
  ...
  const api = new Function('window', 'document', 'sessionStorage', 'location', 'navegaciones', 'permisosAplicados', 'servidor', `
    ...
    return { refreshUserPermissionsFromServer };
  `)(window, document, window.sessionStorage, window.location, navegaciones, permisosAplicados,
  ...
  test('un cambio real de permisos SÍ re-aplica y reafirma la ruta', async () => {
    await api.refreshUserPermissionsFromServer();
    navegaciones.length = 0;
    permisosAplicados.length = 0;

    respuestaDelServidor = { role: 'editor', permissions: { allowed_sections: ['conciliacion'] } };
    await api.refreshUserPermissionsFromServer();

    expect(permisosAplicados).toHaveLength(1);
    expect(navegaciones).toHaveLength(1);
    expect(navegaciones[0].key).toBe('conciliacion');
  });

  // Aunque reafirme la ruta, esto nunca es un clic: no debe mover la pestaña.
  test('al reafirmar la ruta pide conservar la pestaña activa', async () => {
    await api.refreshUserPermissionsFromServer();
    navegaciones.length = 0;
    ...
    expect(navegaciones[0].options).toEqual({ preserveActiveTab: true });
  });
