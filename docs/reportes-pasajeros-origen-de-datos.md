...
El oficio compara dos cierres —el del día pedido y el del día anterior—, como
en la hoja de septiembre con `31/08/2026` y `01/09/2026`. El día anterior
puede caer en el mes previo: el 1 de un mes se compara con el último del mes
anterior, y cualquier otro día con el que le precede.

Cada columna trae sus propios acumulados, contados **hasta su fecha de cierre**.
Si el día pedido no tiene cierre capturado, el oficio lo avisa en pantalla en
vez de saltar a otra fecha.
...
|---|---|---|---|---|
| Subsecretaría | `CIERRE SUBSECRETARIA`: el día pedido y el anterior | `TIPO DE MANIFIESTO` × `TIPO DE OPERACIÓN` | `SUM(TOTAL PAX)` | `COUNT(AEROLINEA)` |
